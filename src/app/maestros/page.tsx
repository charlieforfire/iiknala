...
export default async function MaestrosPage() {
  ...
  const { data: classes } = await adminDb
    .from('yoga_classes')
    .select('id, title, date, time, enrolled, capacity')
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      <h1 className="text-2xl font-light text-stone-800 mb-6">Mis clases</h1>

      {!classes?.length ? (
        <div className="text-center py-16">
          <p className="text-stone-400 text-lg">No tienes clases próximas asignadas.</p>
          <p className="text-stone-400 text-sm mt-2">Contacta a administración para asignar clases a tu perfil.</p>
        </div>
      ) : (
